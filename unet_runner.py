...
from utils_unet import GlomDataset, IoULoss, save_masks
import torch
...
class GlomModel(pl.LightningModule):
    ...
    def __init__(self, arch, encoder_name, in_channels, out_classes, **kwargs):
        ...
        self.metric_values = {'train': [], 'valid': []}
    ...
    def shared_step(self, batch, stage):
        ...
        save_masks(batch, pr_masks = pred_mask, classes = self.out_classes, show = False)
    ...
    def shared_epoch_end(self, outputs, stage):
        ...
        self.metric_values[stage].append(dataset_iou)

        other_stage = 'valid' if stage == 'train' else 'train'
        fig = plt.figure()
        plt.title("train_val_iou")
        stage_col = 'y' if stage == 'train' else 'g'
        other_stage_col = 'y' if other_stage == 'train' else 'g'
        plt.plot(range(len(self.metric_values[stage])), self.metric_values[stage], color = stage_col )
        plt.plot(range(len(self.metric_values[other_stage])), self.metric_values[other_stage], color = other_stage_col)
        plt.show()
        fig.savefig(fname = f'metrics_plot.png')
