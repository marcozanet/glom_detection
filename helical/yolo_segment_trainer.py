...
class YOLO_Trainer_Segmentor(YOLOBase):
    ...
    def __init__(self, 
        ...
        self.weights = 'yolov5s-seg.pt' if self.weights is None else self.weights 
        self.exp_fold = os.path.basename(self.get_exp_fold(os.path.join(self.yolov5dir, 'runs', 'train-seg')))
        now = datetime.now()
...
def test_YOLOSegmentor(): 
    ...
    epochs=1
    dataset = 'hubmap'
    segmentor = YOLO_Trainer_Segmentor(dataset = dataset, 
