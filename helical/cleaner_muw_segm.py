...
class CleanerSegmMuw(ProfilerMUW):
    ...
    def rename_img2lbl(self, fp_img:str): 
        """ Renames image to label. """
        func = lambda fp_img: os.path.join(os.path.dirname(fp_img).replace('images', 'labels'), os.path.basename(fp_img).replace(self.tiles_image_format, self.tiles_label_format))
        return func(fp_img)
    
    def rename_lbl2img(self, fp_lbl:str): 
        """ Renames label to image. """
        func = lambda fp_lbl: os.path.join(os.path.dirname(fp_lbl).replace('labels', 'images'), os.path.basename(fp_lbl).replace(self.tiles_label_format,self.tiles_image_format))
        return func(fp_lbl)
    ...
    def remove_3_points_masks(self) -> None: 
        """ Deletes txt file label and corresponding images, where the instance mask has less than 3 points."""
        ...
        for label in tile_labels: 
            ...
            with open(label, 'r') as f: 
                rows = f.readlines()
            # check number of points is >=3:
            delete = False
            for row in rows: 
                row = row.replace('\n', '')
                items = row.split(' ')
                if len(items) < 7: # e.g. 1 for class, 3 pairs of coords = tot of 3 points
                    delete = True
                    continue
            # delete masks where points <3:
            if delete is True: 
                self.log.info(f"Removing {label}")
                os.remove(label)
                image = self.rename_lbl2img(fp_lbl=label)
                assert image in tile_images, self.log.error(f"image: {image} is not in self.data['tile_images']")
                self.log.info(f"Removing {image}")
                os.remove(image)
                deleted += 1
        
        self.log.info(f"{self._class_name}.{'del_3_points_masks'}: deleted {deleted} masks with less than 3 points ")
    ...
    def __call__(self) -> None:
        ...
        # 8) removing empty images to only have empty_perc of images being empty
        self._remove_perc_()   
        # remove masks with less than 3 points 
        self.remove_3_points_masks()
...
#     cleaner = CleanerSegmMuw(data_root=data_root, 
#                             safe_copy=safe_copy,
#                             wsi_images_like = '*.tif', 
