...
class Converter(Configurator):
    ...
    def _set_attrs(self)->None:
        ...
        self.level = self.params['tiling_level']
        self.stain = self.params['stain']
        self.annotations_in_slide_coords = self.params['annotations_in_slide_coords']
        self.label_format = self.params['label_format']
        self.interpolate_vertices_times = self.params['interpolate_vertices_times']
        return

    
    def _parse_args(self)->None:
        """ Parses class arguments. """
        assert os.path.isdir(self.folder), ValueError(f"'folder': {self.folder} is not a dir.")
        assert self.convert_from in ['json_wsi_mask', 'jsonliketxt_wsi_mask', 'gson_wsi_mask'], f"'convert_from'{self.convert_from} should be in ['json_wsi_mask', 'jsonliketxt_wsi_mask', 'gson_wsi_mask']. '"
        assert self.convert_to in ['json_wsi_bboxes', 'txt_wsi_bboxes', 'geojson_wsi_mask']
        assert self.save_folder is None or os.path.isdir(self.save_folder), ValueError(f"'save_folder':{self.save_folder} should be either None or a valid dirpath. ")
    ...
    def _get_files(self) -> List[str]:
        ...
        print("used converter_base.get-files")
        return files
    ...
    def _rename_all(self) -> None:
        ...
        rename_file = lambda fp : (fp, os.path.join(os.path.dirname(fp), os.path.basename(fp).split(self.stain.upper())[0] + f'{self.stain.upper()}.{os.path.basename(fp).split(".")[-1]}'))
        files = [os.path.join(self.folder, file) for file in os.listdir(self.folder) if ".DS" not in file and os.path.isfile(file) is True]
        old_new_geojson = [(fp,rename_file(fp)) for fp in files ]
        ...
        print(f"used:_clear_ROI_objs_._rename_all ")
        return
    ...
    def _convert_gson2txt(self, gson_file:str) -> str:
        ...
        assert os.path.isfile(gson_file), self.assert_log(f"'gson_file':{gson_file} is not a valid filepath.", func_n=func_n)

        # 1) read in binary and remove extra chars:
        with open(gson_file, encoding='utf-8', errors='ignore', mode = 'r') as f:
            text = f.read()
        text = '[' + text[text.index('{'):]
        json_fp = os.path.join(os.path.dirname(gson_file), os.path.basename(gson_file).replace('.gson', '.json'))
        # 2) convert to json and save:
        # gson_file = gson_file.replace('.mrxs', '')
        # save_file = gson_file.replace('gson', 'json')
        text = json.loads(text)
        with open(json_fp, 'w') as fs:
            json.dump(obj = text, fp = fs)
        self.log.info(f"{self.class_name}.{func_n}: Converter: ✅ WSi .json annotation converted to WSI .txt annotation. ", extra={'className': self.__class__.__name__})
        # # 3) convert using json converter:
        # converted_file = self._convert_json2txt(json_file=save_file)
    ...
    def _remove_empty_pair(self, empty_file:str) -> None: 
        ...
        print(f"used remove empty pair")
        return
    ...
    def _check_annotations(self) -> None:
        ...
        print(f"used check annotations")
        return
    

    # def _convert_gson2json(self, gson_file:str) -> str:
    #     """ Converts file in gson format to geojson for visualization purposes. """
    #     func_n = self._convert_gson2json.__name__
    ...
    #     print(f"used convert gson2geojson")
    #     return  geojson_file
    ...
    def _convert_gson2json(self, gson_file:str) -> str:
        ...
        with open(gson_file, encoding='utf-8', errors='ignore', mode = 'r') as f:
            text = f.read()
        assert '{' in text, self.assert_log(f"{self.class_name}.{func_n}: label {gson_file} is empty!", func_n=func_n)
        text = '[' + text[text.index('{'):]
        json_obj = text.replace("\\", '').replace("/", '')
        json_obj = json.loads(json_obj)
        json_file = os.path.join(os.path.dirname(gson_file), os.path.basename(gson_file).replace('.gson', '.json'))
    ...
    def _convert_labels2json(self)->None:
        """ Checks whether wsi .json labels exist and if they don't, it converts labels other than .geojson in .json. """
        func_n = self._convert_labels2json.__name__
        json_labels = glob(os.path.join(self.folder, '*.json'))
        if len(json_labels)>0:
            return
        self.format_msg(f"⏳ Json labels don't exist. Converting labels to .json.", func_n=func_n)
        label_path_like = os.path.join(self.folder, f"*.{self.label_format}")
        label_files = glob(label_path_like)
        assert len(label_files), self.assert_log(f"No label with path like {label_path_like} found.", func_n=func_n) 
        if self.label_format=='gson':
            for gson_file in label_files:       
                self._convert_gson2json(gson_file=gson_file)
        self.format_msg(f"✅ Converted labels to .json.", func_n=func_n)
    ...
    def _check_already_converted(self, file: str) -> bool:
        ...
        if len(files) > 0:
            ...
            computed = False
        print(f"used _check_already_converted")
        return computed
    ...
    def _convert_hubmap(self, file:str) ->None:
        ...
        self._interpolate_vertices(json_file=json_file)
        if self.verbose_level in ['medium', 'high']: self.log.info(f'{self.class_name}.{func_n} ✅ Interpolating vertices: {os.path.basename(json_file)}')
        print(f"used convert_hubmap")
        return
    
    
    def _interpolate_vertices(self, json_file: str):
        ...
        for json_sample in json_sample_files:
            ...
            for sample_n, gloms in enumerate(data_new['features']):
                ...
                new_vertices = interpolate(vertices=vertices, times=self.interpolate_vertices_times)
                data_new['features'][sample_n]['coordinates'] = new_vertices
            # save new labels:
            with open(json_sample, 'w') as f:
                json.dump(obj = data_new, fp = f)
        print(f"used interpolate_vertices")
        return
    ...
    def _downscale_geojson_file(self, json_file:str):
        ...
        for json_sample in json_sample_files:
            ...
            if len(data)==0:
                self.format_msg(f"File is empty. Skipping downscale geojson file.", func_n=func_n, type='warning')
                continue
        ...
        print(f"downscale geojson files.")
        return
    ...
    def __call__(self) -> None:
        ...
        for file in tqdm(base_names, desc = f"⏳ Converting annotations:"):
            change_format = lambda fp, fmt: os.path.join(os.path.dirname(fp), os.path.basename(fp).split('.')[0] + f".{fmt}")
            geojson_file = change_format(file, 'geojson')
            json_file = change_format(file, 'json')
            if self.verbose_level in ['medium', 'high']: self.log.info(f'{self.class_name}.{func_n} ⏳ Splitting into multiple .json samples: {json_file}')
            self._split_multisample_json(json_wsi_file=json_file, geojson_sample_rect=geojson_file)
            # if self.verbose_level in ['medium', 'high']: self.log.info(f'{self.class_name}.{func_n} ⏳ Downscaling geojson: {json_file}')
            # self._downscale_geojson_file(json_file=json_file)
            if self.verbose_level in ['medium', 'high']: self.log.info(f'{self.class_name}.{func_n} ⏳ Interpolating vertices: {os.path.basename(json_file)}')
            self._interpolate_vertices(json_file=json_file)
            if self.verbose_level in ['medium', 'high']: self.log.info(f'{self.class_name}.{func_n} ✅ Interpolating vertices: {os.path.basename(json_file)}')

        self.format_msg(f'{self.class_name}.{func_n}: ✅ Converted annotations. ', func_n=func_n)
