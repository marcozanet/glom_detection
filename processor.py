...
class Processor():
    ...
    def _split_images_trainvaltest(self, 
        ...
        if self.mode == 'segmentation':
            ...
            train_masks, val_masks, test_masks = mask_list[:train_idx], mask_list[train_idx:val_idx], mask_list[val_idx:]
    ...
    def _makedirs_moveimages(self, images: list,  masks: list = None, labels: list = None) -> None:
        ...
        assert self.mode in ['segmentation', 'detection'], ValueError(f"'mode'={self.mode}, but should be either 'segmentation' or 'detection'.")
        root = self.dst_root
        ...
        for subfold in subfolds_names:
            for subsubfold in subsubfolds_names:
                os.makedirs(os.path.join(root, self.mode, subfold, subsubfold), exist_ok=True)

        # 2) move images and masks
        if self.mode == 'segmentation':
            assert masks is not None, ValueError(f"'masks' is None, but 'mode' is 'segmentation'.")
            ...
            for img_dir, mask_dir, fold in zip(images, masks, subfolds_names):
                for image, mask in tqdm(zip(img_dir, mask_dir), total= len(img_dir), desc= f"Filling {fold} dir"):
                    ...
                    src_img, dst_img = image, os.path.join(root, self.mode, fold, 'images', img_fn)
                    src_mask, dst_mask = mask, os.path.join(root, self.mode, fold, 'masks', mask_fn)
            ...
            assert labels is not None, ValueError(f"'labels' is None, but 'mode' is 'detection'.")
            ...
            for img_dir, label_dir, fold in zip(images, labels, subfolds_names):
                ...
                for label in label_dir:
                    ...
                    src_label, dst_label = label, os.path.join(root, self.mode, fold, 'labels', label_fn)
