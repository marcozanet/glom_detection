...
def get_tiles_masks(slide_folder: str, 
                    out_folder: str, 
                    tile_shape: int = 2048, 
                    tile_step: int = None,
                    plot = False,
    ...
    slides_fp = [os.path.join(slide_folder, slide) for slide in os.listdir(slide_folder) if 'tiff' in slide and 'DS' not in slide]
    print(f'total slides: {slides_fp}')
    ...
    for fp in slides_fp:
        ...
        # patchifying masks:
        print('Patchifying image and mask:')
        ...
        fn = fn.replace('.json', '')
        out_folder_masks = os.path.join(out_folder, fn, 'tiles', 'masks' )
        out_folder_imgs = os.path.join(out_folder, fn, 'tiles', 'images' )
...
if __name__ == '__main__':
    ...
    get_tiles_masks(slide_folder, out_folder)
